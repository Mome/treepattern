...
The python package \textit{ptTools} defines a DNS for pattern matching in trees, which is intended to be applied on python parse trees for code analysis and testing. Tregex is a tool for tree querying and Tsurgen for tree manipulation \cite{levy_tregex_2006}. In \cite{Creating graphs from dependency parse trees} graphs are created from dependency parse trees.
