...
This paper describes a method to extract relations from a representation that incorporate information of various sources. This relations may be visialized in a conceptual graph or fed into an ontology or database.
...
If we consider natural language as a \textit{serialation of thought} that is decoded and encoded by humans in order to communicate and if we define a formal language that can partially express mental content, than there must be a methods that can partially map natural language to our formal language. Furthermore we can see that natural lanuge has a (mostly) modular architecture. Words can be composed syntactically to form a unit with a composed meaning. Linguistics calls this composable units \textit{constiutens} but in different theories different parts of a sententence qualify as a constituent.
...
The root-pattern specifies constrains for a contituent, that forms the root node of subtree within the property tree. 
...
Head Splitting: The rules in the middle applied to the parsetree on the top yield to the graph in the bottom.
...
\begin{sidewaystable}
	...
	\centering
	\caption{Step by step application of the rules.}
	\begin{tabular}{|c|c|c|}
	  \hline  
